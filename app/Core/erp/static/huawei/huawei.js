...
var CargaArchivo= function () {
    ...
    var eventos = function () {
       ...
       $("#modal-check").find("#btnProcesar").on('click',function () {

            var parameters = new FormData();
            parameters.append('action','procesar');
            parameters.append('id', modal_check.find('#id').val());
            ajaxPOST(window.location.pathname,parameters,function (response) {
                  Swal.fire({
                      title: 'Correcto',
                      text: "Proceso termino correctamente",
                      icon: 'success'
                    }).then((result) => {
                          $("#divDescargar").css("display","block");
                          $("#alink").attr("href", response);
                    });
    ...
    var limpiarForm = function () {
        ...
        modal.find('#id_aplicaciones').val('1');
    ...
    var cboTipoProceso = function () {
           ...
           var cboProceso = '';
            ...
            if ($('#id_aplicaciones').val() != ''){
                    var parameters = new FormData();
                    parameters.append('action', 'getProceso');
                    parameters.append('id', $('#id_aplicaciones').val());
                    divProceso.html('');
                    $('#divFiles').html('');
                    ajaxPOST(window.location.pathname,parameters,function (response) {
